...
import { TimeBlock, Service, Appointment } from '../../types';
...
export interface AppointmentCalendarHandle {
    fetchUserAppointments: () => Promise<void>;
}

const AppointmentCalendar = forwardRef<AppointmentCalendarHandle, AppointmentCalendarProps>(({
    mode,
    onBlockClick,
    ...
    }, [mode, providerId, serviceId, daysToShow]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    const fetchServiceAvailability = async (servId) => {
        try {
            ...
            console.log('Service availability loaded:', formattedAvailability);
    ...
    const saveAvailability = async () => {
        if (mode !== 'provider' || !providerId) {
            console.error('Cannot save availability: not in provider mode or providerId is missing');
            return;
        }
        
        try {
            setLoading(true);
            console.log('Saving availability for provider ID:', providerId);
            ...
            console.log('API response from saving availability:', response);
            setSaveSuccess(true);
    ...
    const handleSaveTimeBlock = () => {
        if (mode !== 'provider') {
            console.error('Cannot save time block: not in provider mode');
            return;
        }
        ...
        console.log('Creating new time block for day:', selectedDay);
        ...
        console.log('New time block created:', newBlock);
        ...
        console.log('Updated availability with new block:', newAvailability);
    ...
    const handleBlockClick = (block) => {
        if (mode === 'consumer' && onBlockClick) {
            console.log('Block clicked for booking:', block);
            onBlockClick(block);
        }
    };
    ...
    const getBlockStyle = (block, isUserAppointment = false, appointment) => {
        ...
        const commonStyles = {
            ...
            fontSize: '0.75rem',
            zIndex: 1, // Ensure all blocks have z-index lower than headers
            cursor: 'pointer',
        };
        
        if (appointment) {
            // For provider mode, show different colors based on appointment status
            const statusColors = {
                confirmed: {
            ...
            return {
                ...
                color: colorSet.text,
                zIndex: 2, // Higher than availability blocks but still below headers
                '&:hover': {
    ...
    return (
        <Box sx={{ 
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
                ...
                {(mode === 'provider' || !service) && (
                    <Typography variant="h6" sx={{ fontSize: '1rem' }}>
                        {title || (mode === 'provider' ? 'Your Availability' : 'Available Appointments')}
                    </Typography>
                )}
            ...
            {error && (
                <Alert severity="error" sx={{ mb: 1, py: 0, fontSize: '0.75rem' }}>
                    {error}
                </Alert>
            )}
            
            {saveSuccess && (
                <Alert severity="success" sx={{ mb: 1, py: 0, fontSize: '0.75rem' }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'row', flex: 1, overflow: 'hidden', width: '100%' }}>
                ...
                <Box sx={{ 
                    ...
                    <Box sx={{ 
                        ...
                        <Box sx={{ width: '30px', flexShrink: 0, position: 'sticky', left: 0, zIndex: 2, bgcolor: '#f5f5f5' }}>
                            ...
                            <Box sx={{ 
                                ...
                                position: 'relative',
                                zIndex: 1 // Lower z-index so it scrolls behind the header
                            }}>
                                {timeSlots.map(hour => (
                                    <Box 
                                        key={hour} 
                                    ...
                                    >
                                        <Typography variant="caption" color="text.secondary" sx={{ fontSize: '0.65rem', pt: 0.5 }}>
                            ...
                            {days.map((day, index) => {
                                ...
                                return (
                                    <Box key={index} sx={{ 
                                        flex: `1 1 ${flexBasis}`,
                                        minWidth: 0,
                                        mr: index < days.length - 1 ? 0.5 : 0,
                                        position: 'relative',
                                        display: 'flex',
                                        flexDirection: 'column',
                                        bgcolor: index % 2 === 0 ? 'rgba(0, 0, 0, 0.01)' : 'rgba(0, 0, 0, 0.03)', // Alternate background colors
                                        ...(index < days.length - 1 && {
                                            borderRight: '1px solid rgba(0, 0, 0, 0.12)' // Simple border approach
                                        })
                                    }}>
                                        ...
                                        <Box sx={{ 
                                            ...
                                            width: '100%',
                                            flex: 1,
                                            pr: index < days.length - 1 ? 1 : 0, // Add padding to the right for columns with separators
                                            zIndex: 1, // Lower z-index so content scrolls behind headers
                                            borderRight: 'none' 
                                        }}>
                                            {/* Hour lines */}
                                            {timeSlots.map(hour => (
                                                    ...
                                                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                        ...
                                                        {mode === 'provider' && (
                                                            <IconButton 
                                                                ...
                                                                sx={{ p: 0.25, color: 'rgba(0, 0, 0, 0.5)' }}
                                                            >
            ...
            {mode === 'provider' && (
                <Dialog open={dialogOpen} onClose={handleCloseDialog}>
                    <DialogTitle>Add Availability on {format(selectedDay, 'EEEE, MMMM d')}</DialogTitle>
                    <DialogContent>
            ...
            {mode === 'provider' && (
                <Box sx={{ mt: 0.5, px: 0.5 }}>
                    ...
                    <Box sx={{ display: 'flex', mt: 0.5, flexWrap: 'wrap', gap: 1 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                            <Box component="span" sx={{ 
                                width: 8, 
                                height: 8, 
                                    ...
                                    <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                                        <Chip 
                                            label={selectedAppointment?.status ? 
                                                   ...
                                                   selectedAppointment.status.slice(1) : 'Unknown'} 

...
const ProviderDashboard: React.FC = () => {
    ...
    const [availabilityView, setAvailabilityView] = useState<boolean>(true);
    ...
    useEffect(() => {
        if (providerId) {
            console.log("Provider ID changed, fetching availability data for ID:", providerId);
    ...
    const fetchAvailabilityData = async (provId: number) => {
        try {
    ...
    const handleStatusChange = async (appointmentId: number, status: Appointment['status']) => {
        try {
    ...
    const handleOpenDeleteDialog = (service: Service, event: React.MouseEvent<HTMLElement>) => {
    ...
    const handleAvailabilityChange = async (availability: Record<string, any[]>) => {
        console.log('Availability updated:', availability);
        
        if (!providerId) {
            console.error('Cannot save availability: Provider ID is missing');
            setError('Unable to save availability: Provider ID not found. Please reload the page or log out and back in.');
            return;
        }
